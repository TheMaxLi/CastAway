...
{#if isModalOpen}
	...
	<div class="fixed inset-0 bg-[#00000076] z-10" onclick={close}></div>
	<div
		class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-8 rounded-lg z-20 min-w-[600px] flex flex-col items-center gap-4"
	>
		{#if showImage}
			<img alt="Preview" class="block max-w-full max-h-52 my-4 rounded" bind:this={image} />
		{:else}
			<button class="w-[400px] h-[400px] bg-amber-400" onclick={() => inputRef?.click()}
